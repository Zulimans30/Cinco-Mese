...
fetch('/treelove.svg')
  ...
  .then(svgText => {
    ...
    const heartPaths = allPaths.filter(el => {
      ...
      return style.includes('#ab81f9') || style.includes('#5d1fc1');
    });
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Zulimans";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2026-12-12T11:58:32'); 

  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2025-12-12T:00:00');
...
function playBackgroundMusic() {
  ...
  if (youtubeParam) {
    ...
    if (!helpMsg) {
      ...
      helpMsg.style.color = '#8301b7';
      helpMsg.style.padding = '10px 16px';
      helpMsg.style.borderRadius = '12px';
      helpMsg.style.boxShadow = '0 2px 8px #dedede';
